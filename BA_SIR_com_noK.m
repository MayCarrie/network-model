function [t,S,I] = BA_SIR_com_noK(N,n,tau,gamma,MaxTime,Type)
...
%随机划分10个社区，使用randperm()把N个数随机排序，并倒置为R
idx=randperm(N).';
%X  Y分别为列向量和行向量 
...
while (t<MaxTime & I(end)>0)
    [step,Rate,Status,e]=Iterate(Rate,Status,G,tau,gamma,idx,com_num);
...
%Do the Up-Dating.
function [step,Rate,Status,Event]=Iterate(Rate,Status,G,tau,gamma,idx,com_num);
...
idx(1:tempt)=1;
idx(tempt:2*tempt)=2;
idx(2*tempt:3*tempt)=3;
idx(3*tempt:4*tempt)=4;
idx(4*tempt:5*tempt)=5;
idx(5*tempt:6*tempt)=6;
idx(6*tempt:7*tempt)=7;
idx(7*tempt:8*tempt)=8;
idx(8*tempt:9*tempt)=9;
idx(9*tempt:10*tempt)=10;
...
switch Status(Event)
    ...
    case 2
        switch idx(Event)
            case 1
                f1=find(Status(1:tempt)==1);
                f2=find(Status(1:tempt)==2);
                if(f1)             
                    Rate(f1)=Rate(f1)+1;
                end
                if(f2)
                    Rate(f2)=Rate(f2)-0.01;
                end
            case 2
                f1=find(Status(tempt:2*tempt)==1);
                f2=find(Status(tempt:2*tempt)==2);
                if(f1)             
                    Rate(f1)=Rate(f1)+1;
                end
                if(f2)
            ...
            case 3
                f1=find(Status(2*tempt:3*tempt)==1);
                f2=find(Status(2*tempt:3*tempt)==2);
                if(f1)             
                    Rate(f1)=Rate(f1)+1;
                end
                if(f2)
            ...
            case 4
                f1=find(Status(3*tempt:4*tempt)==1);
                f2=find(Status(3*tempt:4*tempt)==2);
                if(f1)             
                    Rate(f1)=Rate(f1)+1;
                end
                if(f2)
            ...
            case 5
                f1=find(Status(4*tempt:5*tempt)==1);
                f2=find(Status(4*tempt:5*tempt)==2);
                if(f1)             
                    Rate(f1)=Rate(f1)+1;
                end
                if(f2)
            ...
            case 6
                f1=find(Status(5*tempt:6*tempt)==1);
                f2=find(Status(5*tempt:6*tempt)==2);
                if(f1)             
                    Rate(f1)=Rate(f1)+1;
                end
                if(f2)
            ...
            case 7
                f1=find(Status(6*tempt:7*tempt)==1);
                f2=find(Status(6*tempt:7*tempt)==2);
                if(f1)             
                    Rate(f1)=Rate(f1)+1;
                end
                if(f2)
            ...
            case 8
                f1=find(Status(7*tempt:8*tempt)==1);
                f2=find(Status(7*tempt:8*tempt)==2);
                if(f1)             
                    Rate(f1)=Rate(f1)+1;
                end
                if(f2)
            ...
            case 9
                f1=find(Status(8*tempt:9*tempt)==1);
                f2=find(Status(8*tempt:9*tempt)==2);
                if(f1)             
                    Rate(f1)=Rate(f1)+1;
                end
                if(f2)
            ...
            case 10
                f1=find(Status(9*tempt:10*tempt)==1);
                f2=find(Status(9*tempt:10*tempt)==2);
                if(f1)             
                    Rate(f1)=Rate(f1)+1;
                end
                if(f2)
